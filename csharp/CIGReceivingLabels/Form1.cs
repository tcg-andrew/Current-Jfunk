                ...
                else
                {
                    ...
                    if (pd.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                    {
                        for (int x = 0; x < pd.PrinterSettings.Copies; x++)
                        {
                            ...
                            report.SetParameters(new ReportParameter[] { new ReportParameter("PartNum", txtPart.Text), new ReportParameter("Notes", txtNotes.Text), new ReportParameter("Date", DateTime.Now.ToShortDateString()) });
