...
namespace TCGEpicorForExcel2010
{
    ...
    public class Epicor : Office.IRibbonExtensibility
    {
        ...
        private static string GetResourceText(string resourceName)
        {
            ...
            MessageBox.Show(asm.FullName, "Assembly Name", MessageBoxButtons.OK);
            string[] resourceNames = asm.GetManifestResourceNames();
            MessageBox.Show(resourceNames.Length.ToString(), "Resource Count", MessageBoxButtons.OK);
            for (int i = 0; i < resourceNames.Length; ++i)
            {
                MessageBox.Show(resourceNames[i], "Resource Name", MessageBoxButtons.OK);
                if (string.Compare(resourceName, resourceNames[i], StringComparison.OrdinalIgnoreCase) == 0)
                {
                    using (StreamReader resourceReader = new StreamReader(asm.GetManifestResourceStream(resourceNames[i])))
                    {
                        ...
                        {
                            MessageBox.Show("Found", "Resource", MessageBoxButtons.OK);
        ...
        {
            string blah = GetResourceText("Epicor.xml");
            MessageBox.Show(blah, "Test", MessageBoxButtons.OK);
            return blah;
        }
