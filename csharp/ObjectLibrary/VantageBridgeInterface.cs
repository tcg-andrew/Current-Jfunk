...
namespace ObjectLibrary
{
    ...
    public class XmlQuoteParse
    {
        ...
        {
            string name = node.Name.Substring(1);
            if (name == "prodgroup")
            ...
            else if (name == "asmdesc")
            {
                ...
                Locks = node.InnerText == "Yes Locks";
    ...
    public class VantageBridgeInterface
    {
        ...
        public int CreateQuote(string username, string password, string railsid, int quotenum, string custid, string shiptonum, string stname, string stadd1, string stadd2, string stadd3, string stcity, string ststate, string stzip, string csr, string comments, List<QuoteLine> lineitems, decimal total_price, string carrier, int transitdays, string stemail, string stemailcc, string jobref = "", bool skip_price_validation = false)
        {
            try
            {
                ...
                if (!customerInterface.IsShipToValid(ConfigurationManager.AppSettings["EpicorServer"].ToString(), ConfigurationManager.AppSettings["EpicorPort"].ToString(), username, password, custid, shiptonum, stname, stadd1, stadd2, stadd3, stcity, ststate, stzip, stemail, stemailcc))
...
//                        quoteInterface.PrepareReconfig(ConfigurationManager.AppSettings["EpicorServer"].ToString(), ConfigurationManager.AppSettings["EpicorDatabase"].ToString(), ConfigurationManager.AppSettings["EpicorPort"].ToString(), username, password, ConfigurationManager.AppSettings["EpicorUsername"].ToString(), ConfigurationManager.AppSettings["EpicorPassword"].ToString(), quotenum, lineitems);
                        ...
                        quotenum = quoteInterface.CreateQuote(ConfigurationManager.AppSettings["EpicorServer"].ToString(), ConfigurationManager.AppSettings["EpicorPort"].ToString(), username, password, railsid, custid, shiptonum, csr, comments, carrier, transitdays, jobref);

                    foreach (QuoteLine line in lineitems)
                    {
                        if (line.Process)
                        {
                            if (line.Template != "FX" && !partInterface.PartExists(ConfigurationManager.AppSettings["EpicorServer"].ToString(), ConfigurationManager.AppSettings["EpicorPort"].ToString(), username, password, line.ActualPartNum))
                            {
                            ...
                            else
                            {
                                if (line.Template == "FX" && !partInterface.PartExists(ConfigurationManager.AppSettings["EpicorServer"].ToString(), ConfigurationManager.AppSettings["EpicorPort"].ToString(), username, password, line.ActualPartNum))
                                {
                                    try
                                    {
                                        partInterface.CreatePurchasedPart(ConfigurationManager.AppSettings["EpicorServer"].ToString(), ConfigurationManager.AppSettings["EpicorPort"].ToString(), username, password, line.ActualPartNum, line.Description, line.xml.ProdCode, "RGL", "SFLMAINBIN", 352, true, "Lea");
                                        SQLAccess.SQLAccess.SendMail(ConfigurationManager.AppSettings["EpicorServer"].ToString(), ConfigurationManager.AppSettings["EpicorDatabase"].ToString(), ConfigurationManager.AppSettings["EpicorUsername"].ToString(), ConfigurationManager.AppSettings["EpicorPassword"].ToString(), "eng@styleline.com; jfsoftwarellc@gmail.com", "New FX Door Created - " + line.ActualPartNum, "The bridge has automatically created purchased part #" + line.ActualPartNum + ".  Part created with prodcode " + line.xml.ProdCode + ", part class: RGL, supplier: 9310, backflush: true, buyer: Lea.  Please contact Justin Funk with any issues.");
                                    }
                                    catch(Exception ex)
                                    {
                                        throw new Exception("Error creating FX door part or sending email.  Please notify IT:" + ex.Message);
                                    }
                                }

                                NewQuoteLine lineinfo = quoteInterface.CreateLineItem(ConfigurationManager.AppSettings["EpicorServer"].ToString(), ConfigurationManager.AppSettings["EpicorPort"].ToString(), username, password, railsid, quotenum, line.ActualPartNum, line.Description, line.Discount, line.QuoteComment, line.JobComment, line.SystemWeight, (line.custom_mom ? line.xml : null));
                                ...
                                if (partInterface.PartIsSalesKit(ConfigurationManager.AppSettings["EpicorServer"].ToString(), ConfigurationManager.AppSettings["EpicorPort"].ToString(), username, password, line.ActualPartNum))
                                    quoteInterface.UpdateKitComponents(ConfigurationManager.AppSettings["EpicorServer"].ToString(), ConfigurationManager.AppSettings["EpicorPort"].ToString(), username, password, line.ActualPartNum, quotenum, lineinfo);

                                try
                                {
                                    if (partInterface.PartIsNonStock(ConfigurationManager.AppSettings["EpicorServer"].ToString(), ConfigurationManager.AppSettings["EpicorDatabase"].ToString(), ConfigurationManager.AppSettings["EpicorUsername"].ToString(), ConfigurationManager.AppSettings["EpicorPassword"].ToString(), line.ActualPartNum) && line.ActualPartNum != "SHELVING" && !line.custom_mom)
                                    {
                                        if (line.ActualPartNum != "SHELVING" && !line.custom_mom)
                                            lineinfo.revision = partInterface.GetPartInfo(ConfigurationManager.AppSettings["EpicorServer"].ToString(), ConfigurationManager.AppSettings["EpicorPort"].ToString(), username, password, line.ActualPartNum).RevNum;
                                        quoteAssemblyInterface.CreateQuoteAsm(ConfigurationManager.AppSettings["EpicorServer"].ToString(), ConfigurationManager.AppSettings["EpicorPort"].ToString(), username, password, quotenum, lineinfo.linenum, line.ActualPartNum, lineinfo.revision);
                                    }
                                    else if (line.custom_mom)
                                    {
                                        quoteAssemblyInterface.CreateFromXml(ConfigurationManager.AppSettings["EpicorServer"].ToString(), ConfigurationManager.AppSettings["EpicorPort"].ToString(), username, password, quotenum, lineinfo.linenum, line.PartNum, line.xml, ConfigurationManager.AppSettings["EpicorDatabase"].ToString(), ConfigurationManager.AppSettings["EpicorUsername"].ToString(), ConfigurationManager.AppSettings["EpicorPassword"].ToString());
                                    }
                                }
                                catch (Exception ex)
                                {
                                ...
                                quoteInterface.ReadyToQuote(ConfigurationManager.AppSettings["EpicorServer"].ToString(), ConfigurationManager.AppSettings["EpicorPort"].ToString(), username, password, quotenum, lineinfo.linenum, true);
                            }
                        }
                    }
/*                    if (!skip_price_validation)
            ...
            {
                throw new Exception(ex.Message + " (QUOTENUM " + quotenum + ")");
            }
            return quotenum;
        }
