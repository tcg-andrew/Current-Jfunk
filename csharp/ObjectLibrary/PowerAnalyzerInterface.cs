    ...
    public class PowerAnalyzerInterface
    {
        ...
        public static string ParseFrameCode(string partnum)
        {
            try
            {
                ...
                {
                    if (partnum[4] == '-')
                        frame.Append(partnum.Substring(10, 2));
                    else
                        frame.Append("F");
                }
        ...
        public void CreatePowerTableRevision(string server, string database, string username, string password, List<PowerAnalyzerSetting> settings, string revisionComment)
        {
            ...
            try
            {
                connection = SQLAccess.BeginTransaction(server, database, username, password);
                ...
                int revid = Int32.Parse(SQLAccess.GetScalar(server, database, username, password, command, connection));
        ...
        public string[] GetAssemblyDetails(string server, string database, string username, string password, string company, string jobnum, int assemblyseq)
        {
            ...
            try
            {
                ...
                command.Parameters.Add("Jobnum", jobnum);
                command.Parameters.Add("AssemblySeq", assemblyseq);
                DataSet ds = SQLAccess.GetDataSet(server, database, username, password, command);
