                    ...
                    if (pd.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                    {
                        ...
                        for (int x = 0; x < pd.PrinterSettings.Copies; x++)
                        {
                            ...
                            for (int i = 0; i < qty; i++)
                            {
                                ...
                                report.SetParameters(new ReportParameter[] { new ReportParameter("PartNum", ds.Tables[0].Rows[0][0].ToString()), new ReportParameter("JobNum", txt_jobnum.Text), new ReportParameter("Description", ds.Tables[0].Rows[0][1].ToString()), new ReportParameter("Current", (i + 1).ToString()), new ReportParameter("Qty", qty.ToString()), new ReportParameter("Date", DateTime.Now.ToShortDateString()) });
