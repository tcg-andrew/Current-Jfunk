    ...
    public partial class ShopCapacity : Form
    {
        ...
        private void ShopCapacity_Load(object sender, EventArgs e)
        {
            ...
            for (int intY = 0; intY <= 5; intY++)
            {
                ...
                lblCut[intY].Text = "EUnt";
                lblTemper[intY].Text = "EPck";
                lblUnit[intY].Text = "";
        ...
        private void RefreshDays()
        {
            ...
            conn = new SqlConnection("Data Source=" + server + ";Database=" + database + ";Integrated Security=SSPI");
            command = new SqlCommand("[dbo].sp_JobAsmbl_ECOPackUnit_Count", conn);
            command.CommandType = CommandType.StoredProcedure;
            command.Parameters.Add("@Company", SqlDbType.VarChar, 15).Value = "CIG";
            command.Parameters.Add("@Revision", SqlDbType.VarChar, 8).Value = "TN";
            conn.Open();
            reader = command.ExecuteReader();
            ...
            while (reader.Read())
            {
                ...
                aryDays[intIndex].Cut = 0;
                aryDays[intIndex].Temper = 0;
                aryDays[intIndex].Unit = 0;
