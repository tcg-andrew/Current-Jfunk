...
namespace SQLAccess
{
    ...
    {
        public static DataSet GetDataSetSSPI(string server, string database, SqlCommand command)
        {
            try
            {
        ...
        public static string GetScalarSSPI(string server, string database, SqlCommand command)
        {
            try
            {
                using (SqlConnection sqlConnection = new SqlConnection(String.Format("Data Source = {0}; Initial Catalog = {1}; Integrated Security=SSPI;", server, database)))
