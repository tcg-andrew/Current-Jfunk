...
{
    public class LaborInterface : EpicorExtension<LaborImpl, LaborSvcContract>
    {
        ...
        public string GetPlantForResourceGroup(string server, string database, string username, string password, string company, string group)
        {
            string result = "";
        ...
        public string GetPlantForIndirectActivity(string server, string database, string username, string password, string company, string emp)
        {
            string result = "";

            SqlCommand sqlCommand = new SqlCommand("exec [dbo].sp_GetPlantForIndirectActivity @Company, @EmployeeID");
            sqlCommand.Parameters.AddWithValue("Company", company);
            sqlCommand.Parameters.AddWithValue("EmployeeID", emp);

            try
            {
                result = SQLAccess.GetScalar(server, database, username, password, sqlCommand);
        ...
        public bool EmployeeHasActiveSequencyLabor(string server, string port, string username, string password, string employeeID, string jobnum, string asmseq, string oprseq)
        {
            bool result = false;
            OpenSession(server, port, username, password, Session.LicenseType.Default);
            ...
            try
            {
                ...
                if (ds.LaborHedList.Rows.Count > 0)
                {
                    ...
                    {
                        LaborDataSet.LaborDtlRow lrow = laborDS.LaborDtl.Rows[i] as LaborDataSet.LaborDtlRow;
                        if (lrow.ActiveTrans && lrow.JobNum == jobnum && lrow.AssemblySeq == Int32.Parse(asmseq) && lrow.OprSeq == Int32.Parse(oprseq))
                            result = true;
                    }
                }
            }
...
//                ((LaborDataSet.LaborHedRow)laborDS.LaborHed.Rows[0]).ClockOutTime = RoundToNearestQuarter(((LaborHedListDataSet.LaborHedListRow)ds.LaborHedList.Rows[maxIndex]).ActualClockOutTime);
        ...
        public string SmartActivity(string dbserver, string server, string database, string company, string username, string password, string dbusername, string dbpassword, string employeeID, string jobnum, int assemblyseq, int oprseq, decimal qty, decimal scrap, string scrapreason, string scrapreasondesc)
        {
  ...
  //              {
    //                throw new Exception("The user #" + employeeID + " is not logged in.  Log in through MES View first");
      //          }
        //        else
          //      {
                    SqlCommand sqlCommand = new SqlCommand("select * from dbo.fn_SmartActivityOperations(@Company, @Jobnum, @Asm, @Opr, @Qty) order by bomlevel desc, assemblyseq desc, oprseq asc");
                    sqlCommand.Parameters.AddWithValue("Company", company);
                    sqlCommand.Parameters.AddWithValue("Jobnum", jobnum);
                    sqlCommand.Parameters.AddWithValue("Asm", assemblyseq);
                    sqlCommand.Parameters.AddWithValue("Opr", oprseq);
                    sqlCommand.Parameters.AddWithValue("Qty", qty);

                    DataSet opds = SQLAccess.GetDataSet(dbserver, database, dbusername, dbpassword, sqlCommand);

                    foreach (DataRow oprow in opds.Tables[0].Rows)
                    {
                        string job = oprow[0].ToString();
                        int asm = Int32.Parse(oprow[1].ToString());
                        int opr = Int32.Parse(oprow[2].ToString());
                        int comqty = (int)Math.Truncate(Decimal.Parse(oprow[9].ToString()));

                        StartActivity(server, database, username, password, employeeID, job, asm, opr);
                        if (job == jobnum && asm == assemblyseq && opr == oprseq)
                            EndActivity(server, database, username, password, employeeID, job, asm, opr, qty, scrap, scrapreason, scrapreasondesc);
                        else
                            EndActivity(server, database, username, password, employeeID, job, asm, opr, comqty, 0, "", "");
                    }
            //    }
            }
            catch (Exception ex)
